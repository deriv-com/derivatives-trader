 ...
 * Returns array of unique palindromic permutations
 */
function generatePalindromes(str) {
    if (!canFormPalindrome(str)) return [];
    ...
    const permutations = getPermutations(halfChars);
    const palindromes = permutations.map(perm => {
        const firstHalf = perm.join('');
        const secondHalf = perm.slice().reverse().join('');
...
/**
 * Find palindromic dates in a given year
 * Returns array of dates that read the same forwards and backwards
 */
function findPalindromicDates(year) {
    const palindromicDates = [];
    const yearStr = year.toString();

    // Check different date formats
    const formats = ['MMDDYYYY', 'DDMMYYYY', 'YYYYMMDD', 'MMDDYY', 'DDMMYY', 'YYMMDD'];
    ...
    for (let month = 1; month <= 12; month++) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const yearStr2 = yearStr.slice(-2);

            const dateFormats = {
                MMDDYYYY: monthStr + dayStr + yearStr,
                DDMMYYYY: dayStr + monthStr + yearStr,
                YYYYMMDD: yearStr + monthStr + dayStr,
                MMDDYY: monthStr + dayStr + yearStr2,
                DDMMYY: dayStr + monthStr + yearStr2,
                YYMMDD: yearStr2 + monthStr + dayStr,
            };

            for (const [format, dateStr] of Object.entries(dateFormats)) {
                if (isPalindrome(dateStr)) {
                    palindromicDates.push({
...
function runPalindromeExamples() {
    console.log('🔄 Comprehensive Palindrome Library Examples');
    console.log('=============================================\n');

    // Basic palindrome tests
    console.log('📝 Basic Palindrome Tests:');
    const testStrings = ['racecar', 'hello', 'A man a plan a canal Panama', 'race a car', '12321', 'abccba'];
    ...
    console.log('\n📊 First 20 palindromic numbers:');
    console.log(generatePalindromeNumbers(200).slice(0, 20).join(', '));
    ...
    // Generate palindromes from string
    console.log('\n🎯 Generated Palindromes from "aab":');
    const generated = generatePalindromes('aab');
    ...
    // Palindromic dates
    console.log('\n📅 Palindromic Dates in 2024:');
    const palindromicDates = findPalindromicDates(2024);
    palindromicDates.slice(0, 5).forEach(dateInfo => {
        console.log(`${dateInfo.date} (${dateInfo.format}): ${dateInfo.palindrome}`);
    });

    // Generate palindromic words
    console.log('\n📝 Generated 5-letter Palindromic Words:');
    const palindromicWords = generatePalindromicWords(5, 8);
